...
@Slf4j
public class KotConfigurationMigration implements ApplicationRunner {

    private static final String MIGRATION_KEY = "2026-05-28-kot-configuration-v1";
    ...
    public void run(ApplicationArguments args) {
        ...
        addColumnIfMissing("kot_enabled", "BOOLEAN DEFAULT TRUE");
        jdbcTemplate.update("""
                UPDATE app_configurations
                SET kot_enabled = COALESCE(kot_enabled, TRUE)
                """);
        jdbcTemplate.update(
                "INSERT INTO app_migrations (migration_key, applied_at) VALUES (?, CURRENT_TIMESTAMP)",
                MIGRATION_KEY
        );
        log.info("Applied KOT configuration migration");
    }
